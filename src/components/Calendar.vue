        ...
        <tr>
            ...
            <th v-if="showDropdowns" :colspan="showWeekNumbers ? 6 : 5" class="month">
                <div class="row mx-1">
                    ...
                    <input ref="yearSelect" type="number" v-model="year" @blur="checkYear"
    ...
    <tbody>
        ...
        <tr v-for="(dateRow, index) in calendar" :key="index">
            ...
            <slot name="date-slot" v-for="(date, idx) in dateRow">
                <td :class="dayClass(date)" @click="$emit('dateClick', date)"
                    @mouseover="$emit('hoverDate', date)" :key="idx">
                    {{date.getDate()}}
                </td>
            </slot>
        </tr>
...
export default
{
    ...
    methods:
    {
        ...
        changeMonthDate(date, emit = true)
        {
            ...
            this.currentMonthDate = this.$dateUtil.validateDateRange(date, this.minDate, this
                .maxDate)
    ...
    computed:
    {
        ...
        year:
        {
            ...
            set(value)
            {
                ...
                let newDate = this.$dateUtil.validateDateRange(new Date(value, this.month, 1),
        ...
        month:
        {
            ...
            set(value)
            {
                let newDate = this.$dateUtil.validateDateRange(new Date(this.year, value - 1,
        ...
        calendar()
        {
            ...
            for (let i = 0, col = 0, row = 0; i < 6 * 7; i++, col++, curDate.setDate(curDate
                    .getDate() + 1))
            {
                if (i > 0 && col % 7 === 0)
                {
