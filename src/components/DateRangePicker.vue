...
export default {
  ...
  props: {
    ...
    /**
     * Function to fire on every calendar open. Can be used for maxDate update
     */
    onToggleOpen: {
      type: Function,
      default: () => {}
  ...
  data() {
    ...
    data.rangeLabel = this.dateRange.rangeLabel
    ...
    data.showCustomRangeCalendars = data.rangeLabel === "Custom range" ? true : false
  ...
  methods: {
    ...
    clickAway() {
      if (this.open) {
        ...
        this.rangeLabel = this.dateRange.rangeLabel
        // this.open = false
        this.togglePicker(false, true)
      }
    },
    clickRange(range) {
      this.in_selection = false;
      this.showCustomRangeCalendars = false

      if (this.$dateUtil.isValidDate(range.value[0]) && this.$dateUtil.isValidDate(range.value[1])) {
        this.start = this.$dateUtil.validateDateRange(new Date(range.value[0]), this.minDate, this.maxDate)
        this.end = this.$dateUtil.validateDateRange(new Date(range.value[1]), this.minDate, this.maxDate)
        ...
        if (typeof document === "object")
          this.$nextTick(() => {
            ...
            if (!this.alwaysShowCalendars && this.ranges) {
              this.showCustomRangeCalendars = this.rangeLabel === "Custom range" ? true : false
